...
object BankSmsParser {
    private val BANK_SENDERS = setOf(
        ...
        "HDBank","Eximbank","8149","8150","8900","6088","VIB","OCB"
    )

    private val AMOUNT_PATTERNS = listOf(
        Regex("""([\d,.]+)\s*(?:VND|vnd|đ|VNĐ|dong)"""),
    ...
    private val DEBIT_WORDS = listOf("no ","chi ","thanh toan","rut tien","debit","giam","payment","ps no","tien ra","tai khoan no")
    private val CREDIT_WORDS = listOf("co ","nhan ","chuyen den","credit","nap tien","tang","ps co","tien vao","tai khoan co")
    ...
    private fun extractDescription(body: String): String {
        val p = Regex("""(?:ND|noi dung|GD|mo ta|desc|ref|nd)\s*:?\s*(.{3,80})""", RegexOption.IGNORE_CASE)
        return p.find(body)?.groupValues?.get(1)?.trim() ?: body.take(100)
    }

    private fun extractBalance(body: String): Long? {
        val p = Regex("""(?:SD|so du|balance|du kha dung)\s*:?\s*([\d,.]+)""", RegexOption.IGNORE_CASE)
